...
def right_most(tree):
    ...
    if tree.children and len(tree.children)>0:
        return right_most(tree.children[-1])
    else:
        return tree
def left_most(tree):
    if tree is None:
        return None
    if tree.children and len(tree.children)>0:
        return left_most(tree.children[0])
    else:
        return tree

def standardize_punct(tree, nopunct, left=None, right=None): # tree cannot have unary chains
    if nopunct:
    ...
    if tree.children:
        ...
        tree.children = [c for c in tree.children if not (c.word and (c.word in delete_list or any([c.word.endswith(e) for e in DEL_ENDS_TO])))]
        new_children = []
        if len(tree.children):
            for l, c in zip([left]+tree.children[:-1], tree.children):
                try:
                    if not (c.word in ["'"] and not right_most(l).word.endswith('s')):
                        new_children.append(c)
                except:
                        new_children.append(c)
        tree.children = new_children
        if len(tree.children):
            for l, c, r in zip([None]+tree.children[:-1], tree.children, tree.children[1:]+[None]):
                if l is None:
                    l = left
                if r is None:
                    r = right
                standardize_punct(c, nopunct, right_most(l), left_most(r))
    return tree

# def standardize_punct(tree, nopunct, is_last_child=True): # tree cannot have unary chains
#     if nopunct:
#         delete_list = DELETE + PUNCT
#     else:
#         delete_list = DELETE
#     if tree.children:
#         # tree.children = [c for c in tree.children if not (c.label in delete_list)]
#         tree.children = [c for c in tree.children if not (c.word in delete_list)]
#         # if is_last_child and len(tree.children)>0 and type(tree.children[-1].word) is str and tree.children[-1].word in END_DEL:
#         #     tree.children = tree.children[:-1]
#         if len(tree.children)>0:
#             for c in tree.children[:-1]:
#                 standardize_punct(c, nopunct, is_last_child=False)
#             standardize_punct(tree.children[-1], nopunct, is_last_child=is_last_child)
#     return tree
